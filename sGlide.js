...
(function($){
	var methods = {
		...
		startAt: function(pct, animated){
			// return 'num:'+num;
			var self		= this;
			var knob		= this.children('.slider_knob');
			var follow		= this.children('.follow_bar');
		...
		init: function(options){
			// add assets
			this.html('<div class="follow_bar"></div><div class="slider_knob"></div>');
			...
			if (vert && markers && snaps > 0 && snaps < 10){
				$(self.selector+', #'+guid+'_markers').wrapAll('<div class="'+guid+'_vert-marks"></div>');
...
console.log('knob_adjust',knob_adjust);
